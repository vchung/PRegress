...
class PerfTest
  ...
  def run_routine(n_run_id)
    ...
    c_sql = %Q{insert into perfruns (end_time) values (to_date(:1,'MM/DD/YYYY HH24:MI:SS') )}
	c_insert_stmt = @o_conn.prepare_statement(c_sql)
	c_insert_stmt.set_string(1, c_report_end)
	c_insert_stmt.execute()
    @o_conn.commit()
  end
  def get_runlabel()
    ...
    c_sql = %Q{insert into perfruns (run_id, run_label, start_time) values (:1, :2, to_date(:3,'MM/DD/YYYY HH24:MI:SS') )}
	c_insert_stmt = @o_conn.prepare_statement(c_sql)
